...
from pinvgcn.hypergraphs import load_hypergraph_data
from pinvgcn.model import PinvGCN
from pinvgcn.hypergraphpinv import HypergraphPinv
...
parser = argparse.ArgumentParser(description='Perform several runs of training and testing a Pseudoinverse GCN on a hypergraph dataset.')
...
data = load_hypergraph_data(args.dataset)
if 'train_mask' not in data and args.split_size is None:
    raise ValueError('Selected dataset requires specification of a random split size via -s')

tic = timer()
data = HypergraphPinv(rank=args.rank)(data)
data = data.to(device)
